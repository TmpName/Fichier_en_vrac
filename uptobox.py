...
class cHoster(iHoster):
    ...
    def setUrl(self, sUrl):
        ...
        self.token = self.__sUrl.split('/')[-1]
    ...
    def __getMediaLinkByPremiumUser(self):
        ...
        if not self.oPremiumHandler.Authentificate():
            ...
            if (self.stream):
                ...
                #Try getting direct streaming link
                oRequest = cRequestHandler("https://uptobox.com/api/streaming?token=" + UserToken + "&file_code=" + self.GetToken() )
                sHtmlContent = oRequest.request()
                data = json.loads(sHtmlContent)
                liste = data['data']['streamLinks']
                ...
                for aEntry in liste:
                    tmp_qua = aEntry
                    tmp_file = liste[aEntry]
                    for aEntry2 in tmp_file:
                        qua.append(tmp_qua + '-' + aEntry2)
                        url.append(tmp_file[aEntry2] )
                ...
                #Try getting direct download link
                oRequest = cRequestHandler("https://uptobox.com/api/link?token=" + UserToken + "&file_code=" + self.GetToken() )
                sHtmlContent = oRequest.request()
                data = json.loads(sHtmlContent)
